...
function Navbar() {
	...
	const className = visible ? 'active show' : ''
	return (
		<>
			<button className={`navbar-toggler ${className}`} onClick={toggle}>
				<span className="toggler-icon"> </span>
				<span className="toggler-icon"> </span>
				<span className="toggler-icon"> </span>
			</button>
			<div className={`navbar-collapse ${className}`}>
				<ul id="nav" className="navbar-nav mx-auto">
					{[
						...
						<li className="nav-item" key={e[1]}>
							<a className="ud-menu-scroll" onClick={toggle} href={`#${e[1]}`}>
								{e[0]}
							</a>
						</li>
					))}
