...
export default observer(function TotalsScreenTable({
	...
	const normalizedTerms = TotalsStore.result
			...
			{removeNonExistentLessons(TotalsStore.result, SubjectsStore.result).map(
				total => (
					<DataTable.Row key={total.subjectId.toString()}>
						...
						{total.termTotals
							.filter(
								(e, i) => !!e.markCode || i < (normalizedTerms?.length ?? 0),
							)
							.map((term, i) => (
								<DataTable.Cell
								...
								>
									<Mark
										style={{ flex: 0, height: '80%', width: '90%', padding: 0 }}
										duty={false}
										finalMark={term.mark}
										mark={term.avgMark}
										onPress={() => {
											navigation.navigate(Screens.SubjectTotals, {
												termId: term.term.id,
												finalMark: term.mark,
												subjectId: total.subjectId,
											})
							...
							yearTotals?.map((targetYear, i) => {
								...
								if (!year) {
									return (
										<DataTable.Cell
											key={
												i.toString() + targetYear.mark + targetYear.period.id
											}
										>
											<Mark
												style={style}
								...
								return (
									<DataTable.Cell
										key={i.toString() + year.mark + year.period.id}
									>
										<Mark
											style={style}
											duty={false}
